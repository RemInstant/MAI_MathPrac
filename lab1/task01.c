...
status_codes factorial(long long integer, unsigned long long* res)
{
	if (integer < 0 || res == NULL)
	{
		return INVALID_INPUT;
	}
	*res = 1;
...
status_codes sum_natural_numbers(int integer, unsigned long long* res)
{
	if (integer < 1 || res == NULL)
	{
		return INVALID_INPUT;
	}
	*res = integer + 1;
	if (*res > 2 * ULLONG_MAX / integer)
	{
		return OVERFLOW;
	}
	*res /= 2;
	*res *= integer;
	*res += ((integer + 1) & 1) * integer / 2;
	return OK;
}
...
status_codes print_power_table(long long integer)
{
...
int main(int argc, char** argv)
{
	...
	switch (flag[1])
	{
		...
		case 'e': // power table
		{
			switch (print_power_table(integer))
			{
				case OK:
					break;
				case INVALID_INPUT:
