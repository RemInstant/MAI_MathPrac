...
status_code fread_word(FILE* file, char** line)
{
	if (file == NULL || line == NULL)
	{
		return NULL_ARG;
	...
	while (!feof(file) && !is_separator(ch))
	{
		if (iter + 1 == size)
		{
			size *= 2;
...
status_code fskip_separators(FILE* file)
{
	if (file == NULL)
	{
		return NULL_ARG;
...
int ctoi(char ch, int case_sensitive)
{
	if (isdigit(ch))
	{
		return ch - '0';
	...
	{
		if (case_sensitive && ch <= 'Z')
		{
			return ch - 'A' + 10;
		}
		return tolower(ch) - 'a' + 36;
	}
	return -1;
}

char itoc(int number, int case_sensitive)
{
	...
	else if (case_sensitive && number >= 36 && number < 62)
	{
		return 'a' + number - 36;
	}
	return '\0';
